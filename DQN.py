...
EPSILON_END = 0.001  # Terminating epsilon in e-greedy policy
EPSILON_DECAY = 20000  #The decay rate of epsilon in e-greedy strategy
TARGET_UPDATE = 4  #Update frequency of the target network
MEMORY_CAPACITY = 1000000  #Experience playback capacity
HIDDEN_DIM1 = 128  #Network Hidden Layer 1
HIDDEN_DIM2 = 128 #Network Hidden Layer 2
STATE_DIM = 11   #State space dimension
ACTION_DIM = 15   #Action space dimension

def initialize_weights(m):
    if isinstance(m, nn.Linear):
        nn.init.normal_(m.weight, mean=0.0, std=0.01)  # Set weights near 0 with small std
        nn.init.constant_(m.bias, 0.0)  # Bias initialized to 0
...
class DQN(object):
    ...
    def choose_action(self, state):
        self.frame_idx += 1
        epsilon_val =self.epsilon(self.frame_idx)
        if random.random() > epsilon_val:  #Decrease from initial value of 0.9
            with torch.no_grad():
